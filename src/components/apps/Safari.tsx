...
const NavSection = ({ width, section, setGoURL }: NavSectionProps) => {
  ...
  return (
    <div className="mx-auto w-full max-w-screen-md pt-8 px-4">
      ...
      <div className={`mt-3 grid grid-flow-row ${grid}`}>
        {section.sites.map((site: SiteData) => (
          <div key={`safari-nav-${site.id}`} className="h-28 flex flex-col">
            ...
            <span className="mt-2 mx-auto text-sm">
              {site.title}
            </span>
          </div>
        ))}
...
const NavPage = ({ width, setGoURL }: NavProps) => {
  ...
  return (
    ...
    >
      <div className="w-full min-h-full pt-8 backdrop-blur-2xl" style={{ backgroundColor: 'rgb(245 245 245 / 0.8)' }}>
        {/* Favorites */}
        ...
        {/* Privacy Report */}
        <div className="mx-auto w-full max-w-screen-md pt-8 px-4 pb-16">
          <div className="font-medium text-xl sm:text-2xl">
            Privacy Report
          </div>
          <div
            className={`h-16 w-full mt-4 grid ${grid} shadow-md rounded-xl text-sm bg-gray-50/70 dark:bg-gray-600/50`}
          >
...
const NoInternetPage = () => {
  ...
  return (
    ...
    >
      <div className="w-full h-full pb-10 backdrop-blur-2xl flex-center text-c-600" style={{ backgroundColor: 'rgb(245 245 245 / 0.8)' }}>
